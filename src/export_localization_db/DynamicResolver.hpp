...
static Section GetSection(const char* name)
{
    ...
    // the same value (currently ".std"). Resolve the conventional SDK names
    // from section flags as a fallback, while retaining exact-name support
    // for unmodified/older builds.
    ...
    for (int i = 0; i < nt->FileHeader.NumberOfSections; i++)
    {
        ...
        {
            // Prefer the first executable section: it is the main game code
            // section in packed builds and contains the UE implementation.
            if (!semanticMatch.base)
                semanticMatch = { (uintptr_t)m + sect[i].VirtualAddress,
                 ...
                 !isExecutable)
        {
            // GObjects is stored in writable initialized data. Keep the first
            // matching section as the conventional .data replacement.
            if (!semanticMatch.base)
                semanticMatch = { (uintptr_t)m + sect[i].VirtualAddress,
                                   sect[i].Misc.VirtualSize };
        }
...
// ========================================================================
// Phase order: GObjects first (has wait loop ensuring game init), then the
// direct FNamePool decoder, AppendString fallback, and ProcessEventIdx.
