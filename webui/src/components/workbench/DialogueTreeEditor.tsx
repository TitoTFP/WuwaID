...
import { QuestDetail, DialogueLine } from '../../types';
import { GitCommit, CornerDownRight, Plus, Trash2, Edit3, MessageSquare, Layers } from 'lucide-react';

interface DialogueTreeEditorProps {
  quest: QuestDetail;
}

export const DialogueTreeEditor: React.FC<DialogueTreeEditorProps> = ({ quest }) => {
  const [selectedNodeId, setSelectedNodeId] = useState<string>(quest.lines[0]?.id || '');

  const selectedLine = quest.lines.find((l) => l.id === selectedNodeId) || quest.lines[0];
      ...
      <div className="cyber-card p-4 flex items-center justify-between bg-obsidian-900 border-obsidian-800">
        <div>
          ...
          <p className="text-xs text-slate-400 font-mono mt-0.5">
            Kelola alur percakapan, cabang pilihan (Dialogue Choices), dan struktur ID speaker.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
        ...
        <div className="lg:col-span-5 cyber-card p-4 space-y-3 max-h-[600px] overflow-y-auto">
          <div className="flex items-center justify-between pb-2 border-b border-obsidian-800 text-xs font-mono">
            <span className="text-slate-400 font-bold uppercase">Pohon Percabangan Dialog</span>
          ...
          <div className="space-y-2">
            {quest.lines.map((line) => {
              ...
              if (isSeparator) {
                return (
                  <div key={line.id} className="py-2 text-center text-xs font-mono text-cyber-cyan border-y border-obsidian-800">
                    {line.text.id || line.text.en}
                  </div>
                );
              }
                  ...
                  {isChoice && line.options && (
                    <div className="mt-2 space-y-1 pl-3 border-l border-cyber-gold/40">
                      {line.options.map((opt) => (
                        <div key={opt.id} className="text-[11px] text-cyber-gold flex items-center space-x-1">
                          <CornerDownRight className="w-3 h-3 shrink-0" />
                          <span className="line-clamp-1">{opt.text.id || opt.text.en}</span>
                        </div>
                      ))}
                    </div>
                  )}
        ...
        <div className="lg:col-span-7 cyber-card p-5 space-y-4">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[11px] font-mono text-slate-400 block mb-1">Speaker ID:</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-[11px] font-mono text-slate-400 block mb-1">Nama Speaker (EN):</label>
                <input
                  type="text"
            ...
            {/* Teks Sumber EN */}
            <div>
              <label className="text-[11px] font-mono text-slate-400 block mb-1">Teks Dialog Sumber (English):</label>
              <textarea
            ...
            {selectedLine?.type === 'choice' && selectedLine.options && (
              <div className="space-y-2 pt-2 border-t border-obsidian-800">
                ...
                {selectedLine.options.map((opt, idx) => (
                  <div key={opt.id} className="p-3 rounded bg-obsidian-950 border border-obsidian-800 space-y-2">
                    <div className="flex items-center justify-between text-xs font-mono">
                      <span className="text-cyber-gold font-bold">Opsi #{idx + 1} ({opt.id})</span>
                    </div>
                    <input
                      type="text"
                      value={opt.text.id || opt.text.en}
                      readOnly
