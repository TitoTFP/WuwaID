...
// Chart items
const chartRangeSelector = document.getElementById('range-selector');
...
const inspectorUploadId = document.getElementById('inspector-upload-id');
const inspectorUploadMeta = document.getElementById('inspector-upload-meta');
const inspectorFileList = document.getElementById('inspector-file-list');
const btnDownloadZip = document.getElementById('btn-download-zip');
const viewerActiveFilename = document.getElementById('viewer-active-filename');
const viewerCodeBlock = document.getElementById('viewer-code-block');
...
function filterData() {
    ...
    if (activeTab === 'active') {
        if (!query) {
            ...
            const filtered = allPlayers.filter(p => (p.client_id || '').toLowerCase().includes(query) ||
        ...
        if (!query) {
            ...
            const filtered = allLogs.filter(l => (l.id || '').toLowerCase().includes(query) ||
...
function renderHistoryChart(data) {
    ...
    eventKeys.forEach((key, idx) => {
        datasets.push({
            label: key,
            data: points.map((p) => (p.events && p.events[key]) || 0),
...
// ── INSPECTOR DRAWER ──
function openInspector(uploadId) {
    ...
    fetch(`/admin/api/logs/${uploadId}/files`, {
        ...
        .then(res => {
        if (!res.ok)
            throw new Error('Gagal mengambil berkas');
        return res.json();
    })
        .then(d => {
        const files = d.files || [];
        ...
        inspectorFileList.innerHTML = files.map((file) => `
...
function selectFile(filename) {
    ...
    fetch(`/admin/api/logs/${currentUploadId}/files/${urlFilename}`, {
        ...
        .then(res => {
        if (!res.ok)
            throw new Error('Gagal membaca isi berkas log');
        return res.text();
    })
        .then(text => {
